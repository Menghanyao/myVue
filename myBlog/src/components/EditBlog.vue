<template>
  <div id="add-blog">
    ...
    <form v-if="!submited">
      <label>博客标题：</label>
      <input type="text" v-model="blog.title" required="true">
      <label>博客内容：</label>
...
export default {
  ...
  data () {
    return {
      ...
      authors:["Luffy","Zero","Sanji","Nami","Usopu"],
      submited:false,
    }
  },
  methods:{
    post:function(){
      ...
      this.$http.put('https://vue-myblog-532f5.firebaseio.com/posts/'+this.id+'.json',this.blog)
      		.then(function(data){
        console.log(data);
        this.submited=true;
      });
